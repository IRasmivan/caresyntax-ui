...
export class DoctorComponent implements OnInit {
	...
	// On page Load
	ngOnInit() {
		this.dataservice.dataObs$ = 'Doctor';
		...
		this.doctorService.getDoctors(apiurl).subscribe(
			data => {
				...
				for(const doc of data.content){
					this.dataMarts.push( {'id': doc.id, 'doctorName' : doc.name});
				}
			},
			error => {
	...
	onSubmit(doctorAddForm){
		this.postDoctorService(doctorAddForm);
	}

	postDoctorService(doctorAddForm: any) {
		this.saveSuccess = false;
		this.saveError = false;
		this.saveErrorMessage = this.appConfig.generalErrorMsg;
		let apiurl = this.appConfig.uri_add_doctors;
		let pDoctor = {"name": doctorAddForm.value.doctorName};
		this.doctorService.postDoctors(apiurl, pDoctor).subscribe(
			data => {
